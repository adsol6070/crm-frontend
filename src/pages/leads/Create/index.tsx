...
const AddLead = () => {
	...
	const [selectedVisaCategory, setSelectedVisaCategory] =
		useState<DropdownOptions | null>(defaultVisaCategory)
	...
	const [selectedCountryOfInterest, setSelectedCountryOfInterest] =
		useState<DropdownOptions | null>(defaultCountryOfInterest)
	...
	const [selectedState, setSelectedState] = useState<DropdownOptions | null>(
		null
	)
	...
	const [selectedMaritalStatus, setSelectedMaritalStatus] =
		useState<DropdownOptions | null>(defaultMaritalStatus)
	const [phoneValue, setPhoneValue] = useState<string>('')

	const today = new Date()
	const maxPassportExpiryDate = new Date(
		today.getFullYear() + 20,
		today.getMonth(),
		today.getDate()
	)

	const currentSchema: yup.ObjectSchema<any> =
		stepSchemas[step - 1] || yup.object().shape({})
	...
	useEffect(() => {
		if (selectedState && selectedCountry) {
			const districtOptions = City.getCitiesOfState(
	...
	useEffect(() => {
		const fetchSavedData = async () => {
			...
			const savedNationalityOption = sessionStorage.getItem(
				'selectedNationality'
			)
			const savedCountryOfInterestOption = sessionStorage.getItem(
				'selectedCountryOfInterest'
			)
	...
	const onSubmit = async (data: any) => {
		...
		const visaCategory: any = selectedVisaCategory
			? selectedVisaCategory.value
			: null
		const gender: any = selectedGender ? selectedGender.value : null
		const nationality: any = selectNationalityOptions
			? selectNationalityOptions.value
			: null
		const countryOfInterest: any = selectedCountryOfInterest
			? selectedCountryOfInterest.label
			: null
					...
					<Card>
						...
						<Card.Body>
							...
							<FormProvider {...methods}>
								<Form
									className={styles.formStepContainer}
											...
											<Row className={`${styles.customMargin}`}>
												...
												<Col md={6}>
													<FormInput
														...
														errors={errors}
														max={
															maxPassportExpiryDate.toISOString().split('T')[0]
														}
													/>
												</Col>
											</Row>
												...
												<Table striped bordered hover>
													...
													<tbody>
														...
														<tr>
															...
															<td>
																{capitalizeFirstLetter(
																	String(selectNationalityOptions?.value)
																)}
															</td>
														</tr>
														<tr>

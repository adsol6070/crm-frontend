...
const generateInitialPermissions = (modules: any[]): Permissions => {
	const permissions: Permissions = {};
	modules.forEach((module) => {
		const modulePermissions: PermissionOperation = {};
		module.permissions.forEach((permission: { name: string }) => {
			modulePermissions[permission.name] = false;
		});
...
const standardizeRoleName = (role: string) => {
	return role.trim().toLowerCase().replace(/\s+/g, ' ').replace(/[^\w\s]/g, '');
};
...
const PermissionsModal: React.FC<PermissionsModalProps> = ({
	...
	const [localPermissions, setLocalPermissions] = useState<Permissions>(initialPermissions);
	...
	const handleCheckboxChange = (category: string, operation: string) => {
		...
		// Uncheck dependent permissions if the operation is being unchecked.
	...
	const saveChanges = async () => {
		...
		try {
			...
			await permissionService[mode === 'edit' ? 'update' : 'create'](payload, permissionId);
			toast.success(`Permissions successfully ${mode === 'edit' ? 'updated' : 'saved'}.`);
			handleCloseModal();
			onPermissionsChange();
		} catch (error) {
	...
	return (
		<Modal show={showModal} onHide={handleCloseModal} centered size="lg">
			<Modal.Header closeButton>
				<Modal.Title>{mode === 'edit' ? 'Edit' : 'Manage'} Permissions</Modal.Title>
			</Modal.Header>
			<Modal.Body>

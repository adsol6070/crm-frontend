...
const LeadList = () => {
    ...
    const handleSelectedRowsChange = useCallback((selectedRows: Array<{}>) => {
        setSelectedRows(selectedRows);
    }, []);

    const idsInRawData = Array.from(new Set(selectedRows?.map((item: any) => item.original.id)));

    const formattedData: any = {
        leadIds: idsInRawData
    };

    const showDeleteSelectedButton = selectedRows?.length > 0;

...
function TaskCommentService() {
	return {
		...
		createTaskComment: (taskID: string, data: any) => {
			return HttpClient.post(`/taskComment/${taskID}`, data, { headers: getAuthHeaders() })
		},

		// Retrieve all tasks
		getAllTaskComments: async (taskID: string) => {
			return HttpClient.get(`/taskComment/${taskID}`, { headers: getAuthHeaders() })
		},
		...
		getTaskCommentByID: (commentId: string) => {
			return HttpClient.get(`/taskComment/comment/${commentId}`, { headers: getAuthHeaders() })
		},
		...
		deleteTaskCommentByID: (commentId: string) => {
			return HttpClient.delete(`/taskComment/comment/${commentId}`, { headers: getAuthHeaders() })
		},
	}
}

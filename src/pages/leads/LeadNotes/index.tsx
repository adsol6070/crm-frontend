...
const LeadNotes: React.FC = () => {
    ...
    const { createLeadNote, fetchNotes, deleteNotesById, deleteAllLeadNotes, updateLeadNote } = useCreateLeadNote(leadId);
        ...
        <>
            ...
            <Row>
                <Col className="container" md={8}>
                    ...
                    <div>
                        <button onClick={handleAddNote} className={`btn btn-primary my-2 ${styles.addButton}`}>Add Note</button>
                    </div>
                </Col>
            </Row>
            <Row>
                    ...
                    {notes.length === 0 ? (
                        ...
                        <div>
                            <div className='d-flex align-items-center justify-content-end my-1'>
                                <button className={`btn btn-danger ${styles.deleteAllButton}`} onClick={handleDeleteAllNotes}>Delete All Notes</button>
                            </div>
                                    ...
                                    <div key={note.id} className={`${styles.noteCard} card`}>
                                        <div className={styles.cardTopBar}>
                                            {editingNoteId === note.id ? (
                                                <button onClick={handleSaveEditNote} className={styles.iconButton}>
                                                    <i className="ri-save-line"></i>
                                                </button>
                                            ) : (
                                                <>
                                                    <button onClick={() => handleEditNote(note)} className={styles.iconButton}>
                                                        <i className="ri-edit-2-line"></i>
                                                    </button>
                                                    <button onClick={() => handleDeleteNote(note.id!)} className={styles.iconButton}>

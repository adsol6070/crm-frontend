...
const GetLeadWeekReportOnTime: React.FC<GetLeadReportOnTimeProps> = ({ start, end, barColor }) => {
  ...
  const processChartData = (data: any[], start: Date, end: Date) => {
    ...
    const groupedData = filteredData.reduce((acc, curr) => {
      ...
      acc[day] = (acc[day] || 0) + parseInt(curr.lead_count, 10);
      return acc;
    }, {});

    const categories = getDateIntervals(start, end);
    const seriesData = categories.map(day => groupedData[day] || 0);
...
export default GetLeadWeekReportOnTime;
